...
public class ProductControl extends Command {
 ...
 /* If user wants to add a cupcake to cart */
    ...
    // Actions After Buttom is pressed and things are added to cart
    private void cupcakeAfterMatch(HttpServletRequest request, User user, HttpSession session) throws NumberFormatException {
        
        // Initiate get price for bot and top
        CupcakeDataMapper cdm = new CupcakeDataMapper();
        
        int totalPrice = 0;
        
        String pressed = (String) request.getParameter("buy");
        
        if(pressed == null){
            
        } else {
            String topName = (String) request.getParameter("top");
            String botName = (String) request.getParameter("bot");
            
            int botPrice = cdm.getBottomPrice(botName);
            int topPrice = cdm.getTopPrice(topName);
            
            int qty = (int) Integer.parseInt((String)request.getParameter("qty"));
            
            totalPrice = (botPrice + topPrice) * qty;
        }
    }
